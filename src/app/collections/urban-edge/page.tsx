...
import { urbanEdgeSneakers } from '@/lib/products';
...
export default function UrbanEdgeCollectionPage() {
    ...
    const filteredAndSortedProducts = useMemo(() => {
        let products: Product[] = [...urbanEdgeSneakers];
        ...
        if (filters.sizes.length > 0) {
            products = products.filter(p => p.sizes && p.sizes.some(s => filters.sizes.includes(s)));
        }
        if (filters.colors.length > 0) {
            products = products.filter(p => p.color && filters.colors.includes(p.color));
        }
        if (filters.waterproofness.length > 0) {
            products = products.filter(p => p.waterproofness && filters.waterproofness.includes(p.waterproofness));
        }
        if (filters.lacing.length > 0) {
             products = products.filter(p => p.lacingSystem && filters.lacing.includes(p.lacingSystem));
        }
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-center mb-8">
                        <h1 className="text-2xl font-bold text-foreground">
                            Urban Edge Range <span className="text-muted-foreground font-normal">({filteredAndSortedProducts.length})</span>
                        </h1>
                        ...
                        <div className="mb-4 flex items-center gap-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                                {Object.values(filters).flat().map(filter => (
                                    typeof filter === 'string' && <span key={filter} className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-md">{filter}</span>
                                ))}
                            </div>
