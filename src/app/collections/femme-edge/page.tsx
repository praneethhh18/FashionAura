...
import { femmeEdge } from '@/lib/products';
...
export default function FemmeEdgeCollectionPage() {
    ...
    const availableCategories = useMemo(() => {
        const cats = new Set(femmeEdge.map(p => p.category).filter(Boolean) as string[]);
        return Array.from(cats);
    }, []);

    const filteredAndSortedProducts = useMemo(() => {
        let products: Product[] = [...femmeEdge];
    ...
    const activeFilterCount = (filters.colors.length > 0 ? 1 : 0) + 
                              (filters.categories.length > 0 ? 1 : 0) +
                              (filters.priceRange[0] > 0 || filters.priceRange[1] < 8000 ? 1 : 0);
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-center mb-8">
                        <h1 className="text-2xl font-bold text-foreground">
                            Femme Edge Collection <span className="text-muted-foreground font-normal">({filteredAndSortedProducts.length})</span>
                        </h1>
                     ...
                     {activeFilterCount > 0 && (
                        <div className="mb-4 flex items-center gap-2 flex-wrap">
                            ...
                            {filters.priceRange[0] > 0 || filters.priceRange[1] < 8000 ? <span className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-md">Price</span> : null}
                            {filters.colors.map(v => <span key={v} className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-md">{v}</span>)}
                            {filters.categories.map(v => <span key={v} className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-md">{v}</span>)}
