...
import { ethnicWearMen } from '@/lib/products';
...
const categories = ["Kurta Sets", "Sherwanis", "Nehru Jackets", "Juttis"];

export default function MenEthnicPage() {
  ...
  const filteredProducts = useMemo(() => {
    if (selectedCategories.length === 0) {
      return ethnicWearMen;
    }
    return ethnicWearMen.filter(product => {
        if (!product.category || !selectedCategories.length) return true;
        // A bit of a hack since product categories are broad
        return selectedCategories.some(category => {
            if (category === 'Kurta Sets') return product.name.includes('Kurta');
            if (category === 'Sherwanis') return product.name.includes('Sherwani');
            if (category === 'Nehru Jackets') return product.name.includes('Jacket');
            if (category === 'Juttis') return product.name.includes('Jutti');
            return false;
        });
    });
  }, [selectedCategories]);
    ...
    <div className="flex flex-col bg-background min-h-screen">
      ...
      <main className="flex-grow">
         <section className="relative h-[50vh] w-full flex items-center justify-center text-center text-white">
            <Image
                src="/pages/ethnic/men-ethnic.png"
                alt="A man in traditional Indian attire."
                fill
                className="object-cover"
                data-ai-hint="man traditional wear"
                priority
            />
