...
import { streetStyleMen } from '@/lib/products';
...
const categories = ["Hoodies", "Graphic Tees", "Sneakers", "Jackets"];

export default function MenStreetStylePage() {
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  
  const allProducts = useMemo(() => streetStyleMen.filter(p => p.category === 'Streetwear' || p.category === 'Footwear'), []);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return allProducts.filter(product => {
        ...
        return selectedCategories.some(category => {
            ...
            if (category === 'Graphic Tees') return product.name.toLowerCase().includes('tee');
            if (category === 'Sneakers') return product.category === 'Footwear';
            if (category === 'Jackets') return product.name.toLowerCase().includes('jacket');
            return false;
        });
    });
    ...
    <div className="flex flex-col bg-background min-h-screen">
      ...
      <main className="flex-grow">
         <section className="relative h-[50vh] w-full flex items-center justify-center text-center text-white">
            <Image
                src="/pages/street/men-street.png"
                alt="A man in modern street style fashion."
                fill
                className="object-cover"
                data-ai-hint="man street fashion"
                priority
            />
            ...
            <div className="relative z-10 p-8 max-w-4xl">
                 ...
                 <p className="mt-4 text-lg md:text-xl max-w-2xl mx-auto">
                    Bold, authentic, and ready for the city.
                 </p>
            </div>
        </section>
